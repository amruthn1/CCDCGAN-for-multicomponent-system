...
def max_band_gap(y_true, y_pred):
    return K.mean(K.exp(y_pred))
...
class CCDCGAN:
    def __init__(self):
        ...
        self.final_combined.compile(
            loss=losses, loss_weights=lossWeights, optimizer=optimizer
        )
    ...
    def rebuild_constrain(self):
        ...
        model.load_weights('./calculation/model/band_gap_reg.h5')
        ...
        for epoch in range(1, 1 + epochs):
            ...
            if epoch % save_interval == 0:
                if not os.path.exists(
                    GAN_calculation_folder_path + "step_by_step_GAN_model/"
                ):
                    os.makedirs(GAN_calculation_folder_path + "step_by_step_GAN_model/")
                self.generator.save(
                    GAN_calculation_folder_path + "step_by_step_GAN_model/generator.h5"
                )
                self.discriminator.save(
                    GAN_calculation_folder_path
                    + "step_by_step_GAN_model/discriminator.h5"
                )
