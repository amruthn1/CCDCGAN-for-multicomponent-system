...
def combine_for_pre(
    generated_pre_lattice_path="./generated_lattice/",
    generated_pre_sites_path="./generated_sites/",
    generated_crystal_path="./generated_crystal/",
):
...
def combine(
    ...
    filename = os.listdir(generated_pre_sites_path)
    for eachfile in filename:
        if eachfile.endswith(".vasp"):
            try:
                filename = (
                    generated_pre_lattice_path + eachfile.split(".")[0] + ".npy.vasp"
                )
                cell = read(filename)
                cg_cell = cell.get_positions()
                ...
                mind = get_min_distance_for_atoms(
                    generated_crystal_path + eachfile, filetype="vasp"
                )
                if mind < 0.5:
                    print("does not fulfill distance constrain")
                else:
                    write(
                        generated_crystal_path + eachfile[:-5] + ".cif",
                        real_mat,
                        format="cif",
                    )
