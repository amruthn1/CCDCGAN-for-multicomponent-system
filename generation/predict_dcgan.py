...
previous_model_path = "./calculation/"
...
class DCGAN:
    ...
    def rebuild_discriminator(self):
        model = load_model(
            previous_model_path + "step_by_step_GAN_model/discriminator.h5"
        )
        model.summary()
    ...
    def predict(self, epochs, GAN_calculation_folder_path="./calculation/"):
        for epoch in range(1, 1 + epochs):
            ...
            if not os.path.exists(
                GAN_calculation_folder_path + "generated_2d_graph_square"
            ):
                os.makedirs(GAN_calculation_folder_path + "generated_2d_graph_square")
            np.save(
                GAN_calculation_folder_path
                + "generated_2d_graph_square/epoch_%d.npy" % epoch,
                gen_imgs,
            )
